...
const exercise11 = (email) => {
  ...
  const dum = parts[1];
  const companyName = dum.split(".");
  console.log("company name" + companyName[0])
...
const exercise12 = (person) => {
  return person.subordinates.map((subordinate) => exercise11(subordinate.email));
};
...
const exercise22 = (person) => {
  const subOrdinates = person.subordinates.filter(
    (subordinate) => subordinate.subordinates.length > 0
  )
  return subOrdinates.map((subordinate) => subordinate.name);
};
...
const exercise32 = (person) => {
  const numberOfFemales = person.subordinates.filter(
    (subordinate) => subordinate.gender === "female"
  ).length;
  const numberOfMales = person.subordinates.length - numberOfFemales;
...
const exercise32a = (person) => {
  return person.subordinates.reduce(
    (dif, subordinate) => (subordinate.gender === "female") ? ++dif : --dif,
    0
  );
...
const exercise52 = (maxDistance) => {
  ...
  var manager;
  const getTotalPeople = (person) => {
    if (manager != null && distance(person.location, manager.location) <= maxDistance) {
      num++;
    }
    // recursively count total employees of all subordinates of person
    person.subordinates
...
const exercise53 = () => {
  ...
  var manager;
  const getTotalPeople = (person) => {
    if (manager != null && exercise11(person.email) === exercise11(manager.email)) {
